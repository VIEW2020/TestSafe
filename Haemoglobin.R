...
hb.strings <- c("BLOOD HAEMOGLOBIN", "BLOOD/HAEMOGLOBIN", "HAEMOGLOBIN", "HAEMOGLOBIN [G/L]",
              "HB", "HB - HAEMOGLOBIN", "HB  - HAEMOGLOBIN", "ISTAT HAEMOGLOBIN","TOTAL HAEMOGLOBIN")
...
for(year in 2004:2017){
  ...
  DATA <- DATA[OBSC_DESC %in% hb.strings]
  ...
  DATA <- DATA[, c("OBSR_UNITS", "OBSR_RESULT_NUM"):=list(tolower(OBSR_UNITS),
        ...
        # Some results have no units [very few: between <1000 and <30 records for each year of data]. 
        # For haemoglobin, just remove these records. All other records have g/l units
        DATA <- DATA[!is.na(OBSR_UNITS)]  
  
        # Apply range Check
        # Remove results that are >=300 g/l. No lower limit except 0. 
        DATA <- DATA[OBSR_RESULT_NUM>0 & OBSR_RESULT_NUM<300]
  ...
  #       Removes up to 29K records (in 2015)
  ...
  #       Removes up to 125470 records (in 2015)
  DATA <- DATA[order(OBRQ_RECV_DATE_TIMESTAMP, decreasing = T)
               , by=list(VIEW_ENHI_MASTER, OBRQ_RECV_DATE)
               , index:=seq_len(.N)][index==1, !c("index"), with=F]
  
  # viii. Export
  saveRDS(DATA, paste0("source_data/R/TESTSAFE/HAEMOGLOBIN/TS_HAEMO_TOTALPOP_", year, "_v3.rds"))
...
for(year in 2000:2016){
  ...
  DATA <- readRDS(paste0("source_data/R/TESTSAFE/HAEMOGLOBIN/TS_HAEMO_TOTALPOP_", year, ".rds"))

  if(year==2000){

    ALL_HAEMOGLOBIN <- DATA

    } else {

    ALL_HAEMOGLOBIN <- rbind(ALL_HAEMOGLOBIN, DATA)
...
saveRDS(ALL_HAEMOGLOBIN, "source_data/R/TESTSAFE/HAEMOGLOBIN/TS_HAEMOGLOBIN_TOTALPOP_2000_2016.rds")
